...
# Agente do processo CON-05 — Encerramento de Convênio
...
- POP canônico: `pops/CON/CON-05.pop.json` (versão atual 0.2.1, status rascunho)
- Renderizações: `pops/CON/CON-05.md` e `pops/CON/CON-05.bpmn.json` (nunca editar à mão; usar `python3 scripts/render_pop.py CON-05`)
...
- Raias do fluxograma: Assessoria Técnica da Direção Geral (ATDG), Setor demandante, Direção Geral do Campus
- Normativa: A definir
- Interfaces (mapa de contexto): Assessoria Técnica da Direção Geral (ATDG) → Setor demandante (informa); Assessoria Técnica da Direção Geral (ATDG) → Direção Geral do Campus (informa)
...
2. Coletar insumos novos: `python3 scripts/extract_setor.py --setor S02.01-CON --desde 2026-09-03T02:08:02Z --exclui -` e o texto fornecido pelo usuário.
...
6. Aplicar com `python3 scripts/apply_patch.py CON-05 <patch.json>`; validar com `python3 scripts/validate.py`.
