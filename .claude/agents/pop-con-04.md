...
# Agente do processo CON-04 — Prestação de Contas de Convênio
...
- POP canônico: `pops/CON/CON-04.pop.json` (versão atual 0.2.1, status rascunho)
- Renderizações: `pops/CON/CON-04.md` e `pops/CON/CON-04.bpmn.json` (nunca editar à mão; usar `python3 scripts/render_pop.py CON-04`)
...
- Raias do fluxograma: Assessoria Técnica da Direção Geral (ATDG), Setor demandante, Direção Geral do Campus, TCE-PR
- Normativa: A definir
- Interfaces (mapa de contexto): Setor demandante → Assessoria Técnica da Direção Geral (ATDG) (fornece); Assessoria Técnica da Direção Geral (ATDG) → TCE-PR (informa)
...
6. Aplicar com `python3 scripts/apply_patch.py CON-04 <patch.json>`; validar com `python3 scripts/validate.py`.
