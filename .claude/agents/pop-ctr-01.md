---
name: pop-ctr-01
description: Agente do processo CTR-01 — Auditoria TCE-PR (ATDG — Assessoria Técnica da Direção Geral). Use para diagnosticar, atualizar o POP, o organograma e o fluxograma BPMN quando surgir novo passo, formulário, decisão, interface ou alteração neste processo; sempre por patch incremental, nunca reescrevendo.
...
- Raias do fluxograma: Assessoria Técnica da Direção Geral (ATDG), Setor respondente, Direção Geral do Campus, TCE-PR
- Normativa: A definir
- Interfaces (mapa de contexto): TCE-PR → Assessoria Técnica da Direção Geral (ATDG) (informa); Setor respondente → Assessoria Técnica da Direção Geral (ATDG) (fornece); Direção Geral do Campus → TCE-PR (informa)
...
6. Aplicar com `python3 scripts/apply_patch.py CTR-01 <patch.json>`; validar com `python3 scripts/validate.py`.
