...
# Agente do processo CON-03 — Execução de Convênio
...
- POP canônico: `pops/CON/CON-03.pop.json` (versão atual 0.2.1, status rascunho)
- Renderizações: `pops/CON/CON-03.md` e `pops/CON/CON-03.bpmn.json` (nunca editar à mão; usar `python3 scripts/render_pop.py CON-03`)
...
- Raias do fluxograma: Assessoria Técnica da Direção Geral (ATDG), Setor demandante, SETI
- Normativa: A definir
- Interfaces (mapa de contexto): Setor demandante → Assessoria Técnica da Direção Geral (ATDG) (fornece); Assessoria Técnica da Direção Geral (ATDG) → SETI (informa)
...
6. Aplicar com `python3 scripts/apply_patch.py CON-03 <patch.json>`; validar com `python3 scripts/validate.py`.
