...
# Agente do processo CON-02 — Celebração de Convênio
...
- POP canônico: `pops/CON/CON-02.pop.json` (versão atual 0.2.1, status rascunho)
- Renderizações: `pops/CON/CON-02.md` e `pops/CON/CON-02.bpmn.json` (nunca editar à mão; usar `python3 scripts/render_pop.py CON-02`)
...
- Interfaces (mapa de contexto): SETI → Assessoria Técnica da Direção Geral (ATDG) (informa); Assessoria Técnica da Direção Geral (ATDG) → Direção Geral do Campus (aprova); Assessoria Técnica da Direção Geral (ATDG) → Setor demandante (informa)
...
6. Aplicar com `python3 scripts/apply_patch.py CON-02 <patch.json>`; validar com `python3 scripts/validate.py`.
